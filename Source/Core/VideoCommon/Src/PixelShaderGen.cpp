...
static inline void GeneratePixelShader(T& out, DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType, u32 components)
{
	...
	out.Write("  int4 ic0 = int4(" I_COLORS"[1] * 255.0), ic1 = int4(" I_COLORS"[2] * 255.0), ic2 = int4(" I_COLORS"[3] * 255.0), iprev = int4(0, 0, 0, 0);\n"
...
static const char *TEVCMPColorOPTable[16] =
{
	...
	"   (%s&0xFF) + (((%s.r&0xFF) > %s.r) ? (%s&0xFF): int3(0,0,0))",//#define TEVCMP_R8_GT 8
	"   (%s&0xFF) + (((%s.r&0xFF) == %s.r) ? (%s&0xFF): int3(0,0,0))",//#define TEVCMP_R8_EQ 9
	"   (%s&0xFF) + ((dot((%s.rgb&0xFF), comp16) >  dot((%s.rgb&0xFF), comp16)) ? (%s&0xFF): int3(0,0,0))",//#define TEVCMP_GR16_GT 10
	"   (%s&0xFF) + ((dot((%s.rgb&0xFF), comp16) == dot((%s.rgb&0xFF), comp16)) ? (%s&0xFF): int3(0,0,0))",//#define TEVCMP_GR16_EQ 11
	"   (%s&0xFF) + ((dot((%s.rgb&0xFF), comp24) >  dot((%s.rgb&0xFF), comp24)) ? (%s&0xFF): int3(0,0,0))",//#define TEVCMP_BGR24_GT 12
	"   (%s&0xFF) + ((dot((%s.rgb&0xFF), comp24) == dot((%s.rgb&0xFF), comp24)) ? (%s&0xFF): int3(0,0,0))",//#define TEVCMP_BGR24_EQ 13
	"   (%s&0xFF) + int3(max(sign(int3((%s.rgb&0xFF)) - int3((%s.rgb&0xFF))), int3(0,0,0)) * (%s&0xFF))",//#define TEVCMP_RGB8_GT  14
	"   (%s&0xFF) + int3((int3(255,255,255) - max(sign(abs(int3((%s.rgb&0xFF)) - int3((%s.rgb&0xFF)))), int3(0,0,0))) * (%s&0xFF))"//#define TEVCMP_RGB8_EQ  15
};
...
static const char *TEVCMPAlphaOPTable[16] =
{
	...
	"0.0",//6
	"0.0",//7
	"   (%s.a&0xFF) + (((%s.r&0xFF) > (%s.r&0xFF)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_R8_GT 8
	"   (%s.a&0xFF) + (((%s.r&0xFF) == (%s.r&0xFF)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_R8_EQ 9
	"   (%s.a&0xFF) + ((dot((%s.rgb&0xFF), comp16) >  dot((%s.rgb&0xFF), comp16)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_GR16_GT 10
	"   (%s.a&0xFF) + ((dot((%s.rgb&0xFF), comp16) == dot((%s.rgb&0xFF), comp16)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_GR16_EQ 11
	"   (%s.a&0xFF) + ((dot((%s.rgb&0xFF), comp24) >  dot((%s.rgb&0xFF), comp24)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_BGR24_GT 12
	"   (%s.a&0xFF) + ((dot((%s.rgb&0xFF), comp24) == dot((%s.rgb&0xFF), comp24)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_BGR24_EQ 13
	"   (%s.a&0xFF) + (((%s.a&0xFF) >  (%s.a&0xFF)) ? (%s.a&0xFF) : 0)",//#define TEVCMP_A8_GT 14
	"   (%s.a&0xFF) + (((%s.a&0xFF) == (%s.a&0xFF)) ? (%s.a&0xFF) : 0)" //#define TEVCMP_A8_EQ 15
};
