...
static inline void GeneratePixelShader(T& out, DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType, u32 components)
{
	...
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_COLORS, "int4", I_COLORS"[4]");
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_KCOLORS, "int4", I_KCOLORS"[4]");
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_ALPHA, "int4", I_ALPHA);
	...
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_INDTEXMTX, "int4", I_INDTEXMTX"[6]");
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_FOGCOLOR, "int4", I_FOGCOLOR);
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_FOG, "float4", I_FOG"[2]");

	// For pixel lighting - TODO: Should only be defined when per pixel lighting is enabled!
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_PLIGHT_COLORS, "int4", I_PLIGHT_COLORS"[8]");
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_PLIGHTS, "float4", I_PLIGHTS"[32]");
	DeclareUniform(out, ApiType, g_ActiveConfig.backend_info.bSupportsGLSLUBO, C_PMATERIALS, "int4", I_PMATERIALS"[4]");
	...
	if (bHasIndStage)
	{
		...
		if (bpmem.tevind[n].mid != 0)
		{
			...
			{ // s matrix
				...
				out.SetConstantsUsed(C_INDTEXMTX+mtxidx, C_INDTEXMTX+mtxidx);
				out.Write("int2 indtevtrans%d = int2(" I_INDTEXMTX"[%d].ww * int2(uv%d.xy*255.0) * iindtevcrd%d.xx) >> 8;\n", n, mtxidx, texcoord, n);
			...
			{ // t matrix
				...
				out.SetConstantsUsed(C_INDTEXMTX+mtxidx, C_INDTEXMTX+mtxidx);
				out.Write("int2 indtevtrans%d = int2(" I_INDTEXMTX"[%d].ww * int2(uv%d.xy*255.0) * iindtevcrd%d.yy) >> 8;\n", n, mtxidx, texcoord, n);
