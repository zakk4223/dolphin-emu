...
{
	"(float3(iprev.rgb)/255.0)",         // CPREV,
	"(float3(iprev.aaa)/255.0)",         // APREV,
	"(float3(ic0.rgb)/255.0)",           // C0,
	"(float3(ic0.aaa)/255.0)",           // A0,
	"(float3(ic1.rgb)/255.0)",           // C1,
	"(float3(ic1.aaa)/255.0)",           // A1,
	"(float3(ic2.rgb)/255.0)",           // C2,
	"(float3(ic2.aaa)/255.0)",           // A2,
	...
	"(float3(icprev.rgb)/255.0)",        // CPREV,
	"(float3(icprev.aaa)/255.0)",        // APREV,
	"(float3(icc0.rgb)/255.0)",          // C0,
	"(float3(icc0.aaa)/255.0)",          // A0,
	"(float3(icc1.rgb)/255.0)",          // C1,
	"(float3(icc1.aaa)/255.0)",          // A1,
	"(float3(icc2.rgb)/255.0)",          // C2,
	"(float3(icc2.aaa)/255.0)",          // A2,
	"(float3(itextemp.rgb)/255.0)",      // TEXC,
	"(float3(itextemp.aaa)/255.0)",      // TEXA,
...
{
	"(float4(iprev)/255.0f)",            // APREV,
	"(float4(ic0)/255.0f)",              // A0,
	"(float4(ic1)/255.0f)",              // A1,
	"(float4(ic2)/255.0f)",              // A2,
	"(float4(itextemp) / 255.0f)",         // TEXA,
	"(float4(irastemp) / 255.0f)",         // RASA,
	"(float4(ikonsttemp) / 255.0)",       // KONST,  (hw1 had quarter)
	"float4(0.0, 0.0, 0.0, 0.0)", // ZERO
	///added extra values to map clamped values
	"(float4(icprev)/255.0f)",            // APREV,
	"(float4(icc0)/255.0f)",              // A0,
	"(float4(icc1)/255.0f)",              // A1,
	"(float4(icc2)/255.0f)",              // A2,
	"(float4(itextemp) / 255.0f)",          // TEXA,
	"(float4(icrastemp) / 255.0f)",         // RASA,
...
static const char *tevRasTable[] =
{
	...
	"ERROR14", //3
	"ERROR15", //4
	"int4(alphabump * 255.0f, alphabump * 255.0f, alphabump * 255.0f, alphabump * 255.0f)", // use bump alpha
	"int4(alphabump * 255.0f * (255.0f/248.0f), alphabump * 255.0f * (255.0f/248.0f), alphabump * 255.0f * (255.0f/248.0f), alphabump * 255.0f * (255.0f/248.0f))", //normalized
	"int4(0, 0, 0, 0)", // zero
};
...
static inline void WriteStage(T& out, pixel_shader_uid_data& uid_data, int n, API_TYPE ApiType, RegisterState RegisterStates[4], const char swapModeTable[4][5])
{
	...
	if (bHasIndStage)
	{
		...
		{
			out.Write("alphabump = float(iindtex%d.%s & %s) / 255.0f;\n",
					bpmem.tevind[n].bt,
